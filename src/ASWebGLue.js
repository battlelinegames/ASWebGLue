...
export function initASWebGLue(importObject) {
  ...
  importObject.WebGL.createContextFromCanvas = (canvas_id, context_type) => {
    try {
      ...
      let id = WebGL.contextArray.findIndex(gl => gl == null);
  ...
  */
  ...
  importObject.WebGL.linkProgram = (ctx, program) => {
    try {
      ...
      if (
        !WebGL.contextArray[ctx].getProgramParameter(WebGL.programArray[program], WebGL.contextArray[ctx].LINK_STATUS)
      ) {
  ...
  importObject.WebGL.texImage2D = (ctx, target, level, internalformat, format, typ, image_id) => {
    try {
      ...
      `);
      WebGL.contextArray[ctx].texImage2D(target, level, internalformat, format, typ, WebGL.imageArray[image_id]); //WebGL.getArrayView(pixels));
    } catch (err) {
  ...
  // expiramental WebGL2
  importObject.WebGL.blitFramebuffer = (ctx, srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1, dstY1, mask, filter) => {
